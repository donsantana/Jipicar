...
class InicioController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate, UITextFieldDelegate, URLSessionDelegate, URLSessionTaskDelegate, URLSessionDataDelegate, UIApplicationDelegate, AVAudioPlayerDelegate {
    ...
    @IBOutlet weak var origenError: UILabel!
    ...
    {
        let storeInfoURL: String = "http://itunes.apple.com/lookup?bundleId=com.xoait.Jipicar"
    ...
    func SocketEventos(){
        ...
        if self.appUpdateAvailable(){
            ...
            alertaVersion.addAction(UIAlertAction(title: "Si", style: .default, handler: {alerAction in
                ...
                UIApplication.shared.openURL(URL(string: "itms://itunes.apple.com/us/app/apple-store/id1234014455?mt=8")!)
            }))
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        if textField.isEqual(self.origenText){
            self.origenError.isHidden = true
        }else{
            if (self.origenText.text?.isEmpty)!{
                self.origenError.isHidden = false
            }
        }
    ...
    @IBAction func CompartirApp(_ sender: Any) {
        if let name = URL(string: "itms://itunes.apple.com/us/app/apple-store/id1234014455?mt=8") {
