...
class LoginController: UIViewController, UITextFieldDelegate{
    ...
    let manager = SocketManager(socketURL: URL(string: "http://50.116.120.55:5807")!, config: [.log(true), .forcePolling(true)])
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        /*if self.RegistroView.isHidden == false{
            print("aquiiii")
            if textField.isEqual(self.nombreApText){
                self.nombreError.isHidden = true
            }else{
                if textField.isEqual(self.telefonoText){
                    self.telefonoError.isHidden = true
                    if (self.nombreApText.text?.isEmpty)!{
                        self.nombreError.isHidden = false
                    }
                }else{
                    if textField.isEqual(self.claveText){
                        self.claveError.isHidden = true
                        if (self.telefonoText.text?.isEmpty)!{
                            self.telefonoError.isHidden = false
                        }
                    }else{
                        if textField.isEqual(confirmarClavText){
                            self.confirmarError.isHidden = true
                            if (self.claveText.text?.isEmpty)!{
                                self.claveError.isHidden = false
                            }
                        }
                    }
                }
